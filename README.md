...
| 진행에 중요한 이미지 | 캐주얼/하드 모드 선택 화면, 엔딩 신문 헤드라인, 힌트트론 전단지, 팬(선풍기) 서비스 안내 스티커 |
| 배경 간판 일부 | 하수관 청소업체 광고 간판(길거리/하드웨어 스토어 두 시트) |

### ⏳ 한글화 안 된 것

| 항목 | 이유 |
|---|---|
| 배경에 그림으로 그려진 텍스트(간판, 낙서, 그 외 신문·전단지 등) | 대부분 이미지 픽셀에 직접 그려진 텍스트라 번역 범위 밖 (위 6종 제외) |
